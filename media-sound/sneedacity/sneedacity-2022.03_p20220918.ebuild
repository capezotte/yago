...
_GIT_COMMIT="1f69f052f9154f5f4433cdad9afbb80ed6f5b2f8"
SRC_URI="$HOMEPAGE/archive/${_GIT_COMMIT}.tar.gz -> $P.tar.gz"
S="${WORKDIR}/${PN}-${_GIT_COMMIT}"
KEYWORDS="~amd64"

LICENSE="GPL-2"
SLOT="0"
IUSE="alsa ffmpeg +flac id3tag jack +ladspa +lv2 mad ogg oss
	+portmixer sbsms twolame vamp +vorbis +vst"
...
PATCHES=(
	"${FILESDIR}"/freopen.patch
	"${FILESDIR}"/wxGTK-3.2.patch
	"${FILESDIR}"/system-portsmf-header.patch
)
...
#		--disable-dynamic-loading
		...
		-Dsneedacity_use_midi=system    # not really optional, build fails
		-Dsneedacity_use_portsmf=system # same
		...
		-Dsneedacity_use_wxwidgets=system # WE BE SHIPPING 3.2
	)
...
src_install() {
	cmake_src_install
	# Weird stranded executable
	rm -f "$ED/usr/$PN"
	mv "${ED}"/usr/share/doc/{"$PN","$P"}
}
